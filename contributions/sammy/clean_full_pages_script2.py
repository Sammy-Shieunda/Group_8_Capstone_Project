...
def crop_to_paper(img):
    """
    Finds the paper in the photo and cuts out everything around it (hands,
    desk, table edge). Tries a clean 4-corner match first; if the corners
    are rounded, slightly occluded, or the edges aren't perfectly straight,
    it falls back to a rotated bounding box around the largest shape
    instead of giving up. Only returns None when no shape large enough to
    plausibly be the page was found at all.
    """
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    blur = cv2.GaussianBlur(gray, (5, 5), 0)

    # Auto-scale the Canny thresholds to the image's own brightness instead of
    # fixed numbers, so it adapts to darker or lower-contrast photos.
    median = float(np.median(blur))
    lower = int(max(0, 0.66 * median))
    upper = int(min(255, 1.33 * median))
    edges = cv2.dilate(cv2.Canny(blur, lower, upper), np.ones((5, 5), np.uint8), iterations=2)
    ...
    for c in contours:
        ...
        if best_contour is None:
            best_contour = c
        # Try a range of approximation strengths for a clean 4-corner match
    ...
    if best_contour is not None:
        # No clean quadrilateral, but a large shape was found; use its
        # rotated bounding box instead of losing the crop entirely.
