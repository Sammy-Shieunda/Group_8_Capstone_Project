...
def crop_to_paper(img):
    ...
    edges of the page. This works better than edge detection on this
    dataset because the desks are worn, textured, and full of edges of
    their own (scratches, rust, wood grain), which was tripping up plain
    edge detection.

    Assumes the paper is lighter and less saturated (less colorful) than
    what's behind it, true for the green/worn desks here. It will not
    help if the background is also plain white, or if the paper already
    fills the whole frame with no visible background, but in that second
    case there is nothing that needs cropping anyway.
    """
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    h, s, v = cv2.split(hsv)
    mask = ((s < 45) & (v > 120)).astype(np.uint8) * 255
